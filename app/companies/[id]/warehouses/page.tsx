...
export default function WarehousesPage() {
  ...
  useEffect(() => {
    load().catch(() => setError("Impossible de charger les dépôts"));
  }, [load]);
  ...
  async function handleAdd(e: React.FormEvent) {
    ...
    try {
      const res = await fetch("/api/warehouses", {
        ...
        body: JSON.stringify({ companyId, name, address: address || undefined }),
      });
      const data = await res.json();
      if (!res.ok) {
  ...
  return (
    <main style={{ maxWidth: 700, margin: "0 auto", padding: "40px 24px" }}>
      <div style={{ marginBottom: 20, display: "flex", gap: 16 }}>
        ...
        <Link href={`/companies/${companyId}/stock`} style={{ color: "var(--blue)", fontSize: 13, fontWeight: 600 }}>
          Voir le stock →
        </Link>
      </div>
      ...
      <div className="panel" style={{ marginBottom: 24 }}>
        ...
        <form onSubmit={handleAdd}>
          <div className="field">
            ...
            <input value={name} onChange={(e) => setName(e.target.value)} placeholder="Kaloum, Matoto..." required />
          </div>
          <div className="field">
          ...
          <button className="btn" disabled={submitting} type="submit">
            {submitting ? "Ajout..." : "Ajouter le dépôt"}
          </button>
        </form>
      </div>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
        {warehouses?.map((w) => (
          <div key={w.id} className="panel">
            ...
            {w.address && <div style={{ fontSize: 12, color: "var(--text-secondary)" }}>{w.address}</div>}
          </div>
        ))}
      </div>
    </main>
