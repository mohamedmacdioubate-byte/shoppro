...
export default function CompanyProductsPage() {
  ...
  useEffect(() => {
    loadProducts().catch(() => setError("Impossible de charger les produits"));
  }, [loadProducts]);
    ...
    try {
      const res = await fetch("/api/products", {
        ...
        body: JSON.stringify({
          ...
          professionalPrice: professionalPrice ? Number(professionalPrice) : undefined,
        }),
      });
      const data = await res.json();
      if (!res.ok) {
    ...
    <main style={{ maxWidth: 860, margin: "0 auto", padding: "40px 24px" }}>
        ...
        <form onSubmit={handleAddProduct}>
          ...
          <div style={{ display: "flex", gap: 12 }}>
            <div className="field" style={{ flex: 1 }}>
              <label>Prix particulier (GNF)</label>
              <input type="number" min="0" value={basePrice} onChange={(e) => setBasePrice(e.target.value)} required />
            </div>
            <div className="field" style={{ flex: 1 }}>
              <label>Prix professionnel (GNF, optionnel)</label>
              <input type="number" min="0" value={professionalPrice} onChange={(e) => setProfessionalPrice(e.target.value)} />
            </div>
          </div>
          <div className="field">
      ...
      {products?.length === 0 && (
        <div className="panel" style={{ textAlign: "center", color: "var(--text-secondary)" }}>
          Aucun produit pour le moment — ajoutez-en un ci-dessus.
        </div>
      )}
            ...
            <div style={{ textAlign: "right" }}>
              ...
              {p.professional_price && (
                <div style={{ fontSize: 11.5, color: "var(--text-secondary)" }}>
                  Pro : {Number(p.professional_price).toLocaleString("fr-FR")} GNF
                </div>
              )}
            </div>
          </div>
