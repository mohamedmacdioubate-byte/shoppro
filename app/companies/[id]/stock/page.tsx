...
export default function StockPage() {
  ...
  useEffect(() => {
    load().catch(() => setError("Impossible de charger le stock"));
  }, [load]);
  ...
  async function handleAdjust(e: React.FormEvent) {
    ...
    try {
      ...
      if (!res.ok) {
        setAdjustError(extractErrorMessage(data.error));
        return;
      }
      setAdjustQty("");
      await load();
    } catch {
      setAdjustError("Impossible de contacter le serveur");
    } finally {
  ...
  async function handleTransfer(e: React.FormEvent) {
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok) {
        setTransferError(extractErrorMessage(data.error));
        return;
      }
      setTransferQty("");
      await load();
  ...
  return (
    <main style={{ maxWidth: 900, margin: "0 auto", padding: "40px 24px" }}>
      <div style={{ marginBottom: 20, display: "flex", gap: 16 }}>
        ...
        <Link href={`/companies/${companyId}/warehouses`} style={{ color: "var(--blue)", fontSize: 13, fontWeight: 600 }}>
          Gérer les dépôts →
        </Link>
      </div>
      ...
      <div style={{ color: "var(--text-secondary)", fontSize: 13, marginBottom: 24 }}>
        Niveaux de stock par dépôt et par produit
      </div>
      ...
      {warehouses.length === 0 && (
        <div className="panel" style={{ marginBottom: 20, color: "var(--text-secondary)" }}>
          Aucun dépôt pour le moment. <Link href={`/companies/${companyId}/warehouses`} style={{ color: "var(--blue)" }}>Créez-en un</Link> avant d&apos;ajuster le stock.
        </div>
      )}
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginBottom: 24 }}>
        <div className="panel">
          ...
          <form onSubmit={handleAdjust}>
            <div className="field">
              <label>Dépôt</label>
              <select value={adjustWarehouse} onChange={(e) => setAdjustWarehouse(e.target.value)} required>
            ...
            <div className="field">
              <label>Produit</label>
              <select value={adjustProduct} onChange={(e) => setAdjustProduct(e.target.value)} required>
                <option value="">Choisir...</option>
            ...
            <button className="btn block" disabled={adjustSubmitting} type="submit">
              {adjustSubmitting ? "Application..." : "Appliquer"}
            </button>
          </form>
        </div>
        ...
        <div className="panel">
          ...
          <form onSubmit={handleTransfer}>
            <div style={{ display: "flex", gap: 10 }}>
              <div className="field" style={{ flex: 1 }}>
                <label>Depuis</label>
                <select value={fromWarehouse} onChange={(e) => setFromWarehouse(e.target.value)} required>
                  <option value="">Choisir...</option>
                  {warehouses.map((w) => <option key={w.id} value={w.id}>{w.name}</option>)}
                </select>
              </div>
            ...
            <div className="field">
              <label>Produit</label>
              <select value={transferProduct} onChange={(e) => setTransferProduct(e.target.value)} required>
                <option value="">Choisir...</option>
                {products.map((p) => <option key={p.id} value={p.id}>{p.name}</option>)}
              </select>
            </div>
            ...
            <button className="btn block" disabled={transferSubmitting} type="submit">
        ...
        {inventory?.map((row) => (
          <div key={row.id} className="panel" style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            <div>
              ...
              <div style={{ fontSize: 12, color: "var(--text-secondary)" }}>{row.warehouse_name}</div>
            </div>
            <div style={{ textAlign: "right" }}>
              <div style={{ fontWeight: 700, color: row.quantity <= row.min_threshold ? "var(--red)" : "var(--text-primary)" }}>
                {row.quantity} unités
              </div>
              {row.quantity <= row.min_threshold && (
                <div style={{ fontSize: 11, color: "var(--red)" }}>Stock faible (seuil : {row.min_threshold})</div>
              )}
            </div>
          </div>
        ))}
