  ...
  return (
    <main style={{ maxWidth: 700, margin: "0 auto", padding: "40px 24px" }}>
      ...
      <div style={{ color: "var(--text-secondary)", fontSize: 13, marginBottom: 24 }}>
        Livraisons qui vous sont assignées
      </div>
        ...
        {active.map((d) => {
          ...
          return (
            <div key={d.id} className="panel">
              ...
              <div style={{ fontSize: 13, marginBottom: 2 }}>{d.company_name}</div>
              <div style={{ fontSize: 13, color: "var(--text-secondary)" }}>
                {d.customer_name} {d.customer_phone ? `· ${d.customer_phone}` : ""}
              </div>
              {d.address_line && (
              ...
              <div style={{ fontSize: 13, fontWeight: 600, marginTop: 8 }}>{Number(d.total).toLocaleString("fr-FR")} GNF</div>
              {action && (
                <button
                  className="btn block"
                  style={{ marginTop: 12 }}
                  disabled={updating === d.id}
                  onClick={() => advance(d.id, action.next)}
                >
                  {updating === d.id ? "..." : action.label}
                </button>
              )}
            </div>
          );
