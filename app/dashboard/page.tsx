...
type Company = { id: string; name: string; status: string; role_name: string; is_director: boolean; };
    ...
    <main style={{ maxWidth: 900, margin: "0 auto", padding: "40px 24px" }}>
      ...
      {companies?.length === 0 && (
        <div className="panel" style={{ textAlign: "center", color: "var(--text-secondary)" }}>
          ...
          <div style={{ marginTop: 14 }}><Link href="/companies/new" className="btn">Créer ma première entreprise</Link></div>
        </div>
      )}
        ...
        {companies?.map((c) => (
          <div key={c.id} className="panel">
            ...
            <div style={{ display: "flex", flexWrap: "wrap", gap: 12 }}>
              <Link href={`/companies/${c.id}/products`} style={{ fontSize: 12.5, color: "var(--blue)", fontWeight: 600 }}>Produits</Link>
              <Link href={`/companies/${c.id}/warehouses`} style={{ fontSize: 12.5, color: "var(--blue)", fontWeight: 600 }}>Dépôts</Link>
              <Link href={`/companies/${c.id}/stock`} style={{ fontSize: 12.5, color: "var(--blue)", fontWeight: 600 }}>Stock</Link>
              <Link href={`/companies/${c.id}/drivers`} style={{ fontSize: 12.5, color: "var(--blue)", fontWeight: 600 }}>Livreurs</Link>
              <Link href={`/companies/${c.id}/deliveries`} style={{ fontSize: 12.5, color: "var(--blue)", fontWeight: 600 }}>Livraisons</Link>
              <Link href={`/companies/${c.id}/promotions`} style={{ fontSize: 12.5, color: "var(--blue)", fontWeight: 600 }}>Promotions</Link>
